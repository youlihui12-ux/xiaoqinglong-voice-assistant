...
4. Copy `.env.example` to `.env` and fill local values, including `XIAOQINGLONG_API_TOKEN` for controlled panel actions. Existing installs may keep Doubao keys in `doubao-asr-frontdoor.env`; `.env` overrides legacy values.
